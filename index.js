...
const pricing = new Map(); // guildId -> { renewalPrices: [], mentionPrices: { everyone: 0, here: 0, shop: 0 }, command: '', creditReceiver: null }
...
client.on('interactionCreate', async interaction => {
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'تسطيب-أسعار-التجديد') {
            ...
            if (!pricing.has(guildId)) {
                pricing.set(guildId, { renewalPrices: [], mentionPrices: { everyone: 0, here: 0, shop: 0 }, command: 'C', creditReceiver: null });
            }
        ...
        if (commandName === 'تسطيب-امر-الدفع') {
            ...
            pricing.get(guildId).creditReceiver = interaction.options.getUser('مستلم-الرصيد')?.id || null;
    ...
    if (interaction.isButton()) {
        // التحقق من أن المستخدم مخول لاستخدام الأزرار في هذه الغرفة
        const roomData = tempRooms.get(interaction.channel.id);
...
منشن متجر: ؟`;
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId.startsWith('renewal_modal_')) {
            ...
            let paymentCommand = `${command} ${price}`;
            if (creditReceiverId) {
                paymentCommand += ` <@${creditReceiverId}>`;
            } else {
                paymentCommand += ` <@${interaction.user.id}>`;
            }
        ...
        if (message.channel.name.startsWith('تجديد-متجر-') || message.channel.name.startsWith('شراء-منشن-')) {
            ...
            if (message.channel.name.startsWith('شراء-منشن-') && !message.content.includes('تحويل')) {
                ...
                if (totalPrice > 0) {
                    ...
                    let paymentCommand = `${command} ${totalPrice}`;
                    if (creditReceiverId) {
                        paymentCommand += ` <@${creditReceiverId}>`;
                    } else {
                        paymentCommand += ` <@${message.author.id}>`;
                    }
